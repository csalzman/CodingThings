...
#Notes: Each room will need an array of rooms that go off of it
...
#to print the player_hp use 'player_hp.reduce(:+)'
...
def choices
  ...
  if todo == "Look"
    check_room
    choices
  #Checks status and then starts choices up again
  elsif todo == "Status"
    status
    choices
...
#Takes in all directions from the room in question. Displays them and gets a choice from the player
...
#Will move you from one room to the next
def move(direction)
  puts "You are moving #{direction}."
  eval direction.downcase.gsub(" ", "_")
end

#Start room
def start
  puts "You are in a room inside of a ruined castle. To the West is the entryway of the castle. To the South a doorway opens into a field, the moon hovering above it. To the East, a Prison Guard room, and to the North a dark hallway that leads to the Dungeon."
  choices
